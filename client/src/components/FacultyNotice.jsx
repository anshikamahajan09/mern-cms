  ...
  return (
    <div className=" md:pl-64 p-7 w-full flex flex-col gap-16">
      ...
      <div className="flex w-full flex-col gap-12">
        ...
        <div className="w-3/4">
          <h1 className="font-semibold text-3xl mb-8">All other notice</h1>
          {recentAnnouncements.map(
            (announcement, index) =>
              announcement.userId !== currentUser._id && (
                <div key={index} className="w-full flex flex-col gap-4 mt-4 border border-gray-600 rounded-lg p-4">
                  <div className="flex items-center gap-4">
                    <div className=" text-lg font-semibold ">{announcement.title}</div>
                    <div className="text-gray-400 text-xs italic">
                      {new Date(announcement.createdAt).toLocaleDateString(
